...
static int myfs_file_open(struct inode *inode, struct file *file)
{
	...
	struct user_namespace *this_user_ns = current_user_ns();
	// uid 0 is root
	if(!inode_permission(this_user_ns, inode, MAY_READ|MAY_WRITE) && !current_uid().val == 0)
	{
...
static int __init myfs_init(void)
{
	...
	myfs_creat_file("two", S_IFREG|S_IRUGO, pslot, NULL, &myfs_file_operations);
